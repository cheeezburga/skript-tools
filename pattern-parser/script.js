...
function displayStatistics() {
	const { totalPatterns, longestPattern, shortestPattern, averagePattern, uniqueParseTags } = allData;
	...
	if (uniqueParseTags.length > 0) stats.push({ label: "Unique Parse Tags", value: uniqueParseTags.join(", ") });
...
function showPage(page) {
	...
	if (endIndex > allResults.length) endIndex = allResults.length;
...
function positionTooltip(e) {
	...
	if (x + tooltipWidth > pageWidth) x = e.clientX - tooltipWidth - 15;
	if (y + tooltipHeight > pageHeight) y = e.clientY - tooltipHeight - 15;
